...
router.get('/status', verifyToken, (req, res) => res.json(getStatus(req.user.tenantId)));
router.get('/plans',  (_req, res) => res.json(PLANS));

router.post('/charge', verifyToken, (req, res) => {
  try {
    ...
    const charge = createPixCharge(req.user.tenantId, plan, referral_code);
    res.status(201).json(charge);
  } catch(err) { res.status(500).json({ error: err.message }); }
});

router.get('/charges', verifyToken, (req, res) => {
  res.json(query('SELECT * FROM pix_payments WHERE tenant_id=? ORDER BY created_at DESC LIMIT 20', [req.user.tenantId]));
});

router.post('/confirm/:paymentId', verifyToken, (req, res) => {
  try {
...
router.get('/referral', verifyToken, (req, res) => {
  const user = queryOne('SELECT referral_code, referred_by FROM users WHERE id=?', [req.user.id]);
  const sub  = queryOne('SELECT referral_pct_credit, referral_months_credit FROM subscriptions WHERE tenant_id=?', [req.user.tenantId]);
